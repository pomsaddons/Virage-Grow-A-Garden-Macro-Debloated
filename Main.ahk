...
global GAME_PASS_IDS := [0]
EnvGet, LOCAL_COMPUTER_NAME, ComputerName
global WEB_APP_URL := "https://example.com"
global webhookURL
...
{	global webhookURL
	...
	{		whr := ComObjCreate("WinHttp.WinHttpRequest.5.1")
		...
		{			isValid := 1
			...
			if (k#kkk#%f@k@%fff@kkff@f = "privateserver" && (status >= 200 && status < 400))
			{				isValid := 1
			}
	}	catch
	{		isValid := 0
	...
	{		if (k#kk%ffkf%k#ff%@fkkk#%f@kkff@f = "webhook")
		{			if (isValid && webhookURL != "")
			...
			if (k#kkk#%f@f@#f%fff@%f##f@fk@%kkff@f = "privateserver")
...
{	global SavedSpeed
	...
	{		Loop, Parse, #kk@#f%kfkkk#%f#%f##f@fk@%@f@ffk
		...
		if (k#%#k@kff%%kk#fk@%#kf#kkff#k@k = "calculate")
		{			previousIndex := f@%kfk@%f#@f#kf@kfk@(currentArray, indexItem)
			...
			{				f%k#kfk#k@%k#kfkkk%fk#kkk%k#fff@kk(f%f#k@k@%f#ffk%#kf#ff%f@ffff#ff@)
				...
				if (ff#ff%fk#ff@%kf@ffff#ff@ = "down")
				{					FileAppend, % "sendCount: " . sendCount . "`n", debug.txt
					...
					{						fk#kfk%kkk@#f%kkk#fff%k@ffff#f%@kk("Left")
						...
						if ((currentArray.Name = "seedItems") && (UINavigationFix))
		...
		{			if (f%f@#f#kkk%%#k@k#kk#%f#ffkf@ffff#ff@ = "up")
			{				if (f#@kffk%#kk#@f%@kf@k = "Honey" && UINavigationFix)
				...
				if (ff#ffk%#k@kff%f%kkkkfk#f%@ffff#ff@ = "down")
...
{	if (kk@f%#kk#k#k#%k@k#@f@f#f = "\")
	{		Send, \
		...
		if (kk@fk%#k#k@fkf%@k#@f@f#f = "#" || kk@%ff@k#f%fk@k#@f@f#f = "[")
...
{	FileAppend, % "Searching " . @k@kk%#kk#@f%#f@fffkkf.Name . " for " . @kf%#kk#k#k#%##k@f@f#f . "`n", debug.txt
	...
	{		return 1
		...
		if (k#@f@k@%@kk#%f%#kkk@k%#ff#f@fk = "bool")
		{			return false
		}
}
...
{	global UINavigationFix
	...
	{		fkfkk%#kf#ff%@@k@kkk("150524150505305", 0)
		...
		{			fkf%k@kk@k%kk%ff#k@k@k%@@k@kkk("4335505541555055", 1, 1)
			...
			if (k#f%kkf@kfff%k@f#ff%k@ffffkk%#@fk@ = "pollinated")
			{				fk%fkk#%fkk@@k@kkk("22115505544444444441111111155055", 1, 1)
			}
		else
			if (k#f%#kk#k#k#%k@%k#ff%f#ff#@fk@ = "pollen")
...
{	Loop, 5
	...
	{		@fk@%@fkkk#%kf%kfk#f#%f#k#kkf@#k(midX + 0.4, midY - 0.1)
		...
		if (kkk#f%k@fkkf%@fkf#fkf@ff = "honey")
...
Gui, Font, s10 cWhite Bold, Segoe UI
Gui, Add, Text, x100 y70 w200 h24, pom
Gui, Font, s8 cWhite Italic, Segoe UI
Gui, Add, Text, x100 y96 w200 h16, Holy.. your obfuscator is buns
Gui, Font, s8 cWhite, Segoe UI
Gui, Add, Text, x40 y130 w200 h40, This took 20 mins to crack lmfao
Gui, Add, Text, x40 y150 w200 h40, Selling AHK scripts for 5 dollars is actually wild vro
Gui, Font, s10 cWhite Bold, Segoe UI
...
{	group := SubStr(A_GuiControl, 10)
	...
	if (RegExMatch(A_GuiControl, "^(Seed|Gear|Egg|Honey)Item\d+$", m))
	{		group := m1
