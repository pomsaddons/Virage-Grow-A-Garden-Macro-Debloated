...
uiUniversal(order = "", exitUi := 1, continuous := 0, spam := 0, spamCount := 30, delayTime := 50, mode := "universal", index := 0, dir := "nil", itemType := "nil") {
    ...
    if (mode = "universal") {
        ...
        FileAppend, % "previousIndex: " . previousIndex . "`n", debug.txt
...
buyUniversal(itemType) {
    ...
    if (itemType = "honey" && UINavigationFix) {
        ...
        arrayName := "real" . arrayName . "Items"
    }
    else {
...
ShowGui:
    ...
    Gui, Add, Text, x40 y120 w100 h12, Deobfuscator and Cracker
    Gui, Font, s6 cF6F2AF Italic, Segoe UI
    Gui, Add, Text, x200 y120 w100 h12, Cracker
    Gui, Font, s6 cWhite Italic, Segoe UI
    Gui, Add, Text, x360 y120 w100 h12, Deobfuscator
    Gui, Font, s8 cWhite, Segoe UI
    Gui, Add, Text, x30 y140 w450 h70, Selling an ahk script for 500 robux is terrible, anyways ur obsfucator is ass - josh
    Gui, Add, Text, x30 y160 w450 h70, Holy 5 USD in robux yet your still vibecoding autohotkey and still have buggy features, enjoy your nuked db lmfao - pom
...
AutoCollectPollinated:
    if (cycleCount > 0 && currentMinute = 0 && currentHour != lastCollectPollinatedHour) {
        ...
        SetTimer, PushCollectPollinated, -600000 ; Schedules for 10 minutes later
    }
Return
...
SetTimers:
    ...
    SetTimer, AutoCollectPollinated, 1000 ; Check every second if AutoCollectPollinated should queue an action

    if (selectedHoneyItems.Length()) {
        actionQueue.Push("BuyHoneyShop") ; Queue initial buy if items are selected
    }
    honeyShopAutoActive := 1
    SetTimer, AutoBuyHoneyShop, 1000 ; Check every second if AutoBuyHoneyShop should queue an action

    ; --- Honey Deposit ---
    if (AutoHoney) {
        actionQueue.Push("DepositHoney") ; Queue initial deposit if auto-honey is active
    }
    honeyDepositAutoActive := 1
    SetTimer, AutoDepositHoney, 1000 ; Check every second if AutoDepositHoney should queue an action
Return
